...
function BusinessesNavIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
function ExperiencesNavIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
/* CSS stagger delay classes — replaces framer-motion (~32KB savings) */
const staggerDelays = [
  'animate-fade-up [animation-delay:150ms]',
  'animate-fade-up [animation-delay:210ms]',
  'animate-fade-up [animation-delay:270ms]',
  'animate-fade-up [animation-delay:330ms]',
  'animate-fade-up [animation-delay:390ms]',
];
...
export function WelcomeHero({ items }: WelcomeHeroProps) {
  ...
  // Default to 'morning' for SSR so image always renders in initial HTML (LCP fix)
  ...
  const greeting = t(`${timeOfDay}.greeting`);
  const emoji = t(`${timeOfDay}.emoji`);

  const CATEGORIES = [
    { key: 'nearMe', href: '/discover', icon: NearMeIcon, color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-950/30' },
    { key: 'businesses', href: '/businesses', icon: BusinessesNavIcon, color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-950/30' },
    { key: 'experiences', href: '/tourism', icon: ExperiencesNavIcon, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-950/30' },
    { key: 'events', href: '/events', icon: EventsNavIcon, color: 'text-purple-500', bg: 'bg-purple-50 dark:bg-purple-950/30' },
    { key: 'stays', href: '/rentals', icon: StaysNavIcon, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-950/30' },
  ];
    ...
    <header className="relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0">
          ...
          {/* Image always rendered for LCP — same image for all time-of-day configs */}
          <div
        ...
        <div className="relative z-10 px-4 pt-8 pb-7 sm:pt-10 sm:pb-9">
          <div className="max-w-2xl mx-auto">
            {/* SEO: H1 for primary keyword targeting */}
            ...
            <form onSubmit={handleSearch} className="animate-fade-up delay-100">
              ...
              {/* Hidden submit button ensures Enter key triggers form submission in all browsers */}
              <button type="submit" className="sr-only" aria-hidden="true">{t('searchButton')}</button>
            </form>
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            {CATEGORIES.map((cat, i) => {
              const Icon = cat.icon;
              return (
                <div key={cat.key} className={`opacity-0 ${staggerDelays[i] || staggerDelays[0]}`} style={{ animationFillMode: 'forwards' }}>
                  <Link
                    href={cat.href}
                  ...
                  >
                    ...
                    <span className="text-[11px] font-semibold text-gray-700 dark:text-gray-300 leading-none">{tHome(`categories.${cat.key}`)}</span>
                  </Link>
                </div>
              );
            })}
