...
interface TourismExperienceCreateFormProps {
  userId: string
  categories: Array<{ id: string; name: string; slug: string }>
  regions: Array<{ id: string; name: string; slug: string }>
}

export function TourismExperienceCreateForm({ userId, categories, regions }: TourismExperienceCreateFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    tourism_category_id: '',
    experience_type: 'tour' as 'tour' | 'activity' | 'attraction' | 'accommodation' | 'service',
    duration: '',
    difficulty_level: 'easy' as 'easy' | 'moderate' | 'challenging' | 'extreme',
    group_size_min: '',
    group_size_max: '',
    age_requirement: '',
    region_id: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (createError) {
        ...
        setError(`Failed to create experience: ${createError.message}`)
      } else {
