...
export function TourismPhotoUpload({ experienceId, existingPhotos }: TourismPhotoUploadProps) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check if user is at max photos (10 for tourism experiences)
    if (photos.length >= 10) {
      setError('Maximum 10 photos allowed for tourism experiences')
      return
    }
    ...
    try {
      ...
      if (dbError) {
        ...
        await supabase.storage.from('tourism-photos').remove([fileName])
        setUploading(false)
        return
      }
  ...
  const handleDelete = async (photoId: string, imageUrl: string) => {
    ...
    try {
      ...
      const urlParts = imageUrl.split('tourism-photos/')
      ...
      {photos.length < 10 && (
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-2">
            {photos.length}/10 photos • Max 5MB • JPG, PNG, WebP
          </p>
        </div>
      )}
