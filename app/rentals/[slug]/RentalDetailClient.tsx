...
import {
  BedDouble, Bath, Users, Maximize, MapPin, Home,
  ...
  MessageSquare, ExternalLink, Crown, BadgeCheck
} from 'lucide-react'
...
export function RentalDetailClient({
  ...
  const houseRules = rental.house_rules || []
  const utilitiesIncluded = rental.utilities_included || []
  ...
  const averageRating = rental.average_rating || 0
      ...
      {isGalleryOpen && (
        <div className="fixed inset-0 z-[100] bg-black">
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            <Image
              src={allPhotos[galleryPhotoIndex].image_url}
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-2 text-white/90 text-base sm:text-lg mb-4">
              ...
              <span className="font-medium">
                {rental.location_details && `${rental.location_details}, `}
                {rental.regions?.name}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/30 backdrop-blur-xl border border-white/20">
                ...
                <span className="text-white font-bold text-sm">{rental.bedrooms} Beds</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/30 backdrop-blur-xl border border-white/20">
                <Bath className="w-4 h-4 text-white" />
                <span className="text-white font-bold text-sm">{rental.bathrooms} Baths</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/30 backdrop-blur-xl border border-white/20">
                <Users className="w-4 h-4 text-white" />
                <span className="text-white font-bold text-sm">{rental.max_guests} Guests</span>
              </div>
              {totalReviews > 0 && (
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="hidden sm:block">
                ...
                <p className="text-2xl font-black text-gray-900">
                  GYD {rental.price_per_month?.toLocaleString()}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="lg:hidden bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-6 text-white">
              ...
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-4xl font-black">GYD {rental.price_per_month?.toLocaleString()}</span>
                <span className="text-lg">/month</span>
              </div>
              {rental.price_per_week && (
                <p className="text-white/90 mb-1">GYD {rental.price_per_week.toLocaleString()}/week</p>
              )}
              {rental.security_deposit && (
                <div className="mt-4 pt-4 border-t border-white/30">
                  <p className="text-sm"><span className="font-bold">Security Deposit:</span> GYD {rental.security_deposit.toLocaleString()}</p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { icon: Eye, label: 'Views', value: rental.view_count || 0, color: 'blue' },
                { icon: MessageCircle, label: 'Inquiries', value: rental.whatsapp_clicks || 0, color: 'green' },
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100">
                              ...
                              <span className="text-xs font-bold text-amber-700">{review.overall_rating.toFixed(1)}</span>
                            </div>
                          </div>
          ...
          <div className="hidden lg:block lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-lg">
                ...
                {rental.price_per_week && (
                  <p className="text-sm text-gray-600 mb-2">GYD {rental.price_per_week.toLocaleString()}/week</p>
                )}
                ...
                {rental.security_deposit && (
                  <div className="mb-6 p-4 bg-blue-50 rounded-xl border border-blue-200">
                    <p className="text-sm text-gray-700 font-bold">
                      ...
                      GYD {rental.security_deposit.toLocaleString()}
                    </p>
                  </div>
                )}
        ...
        {similarRentals.length > 0 && (
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {similarRentals.map((similar: any) => {
                const similarPhoto = similar.rental_photos?.find((p: any) => p.is_primary)?.image_url || defaultImage
                return (
                  <Link
                    key={similar.id}
                    ...
                    <div className="relative h-48">
                      ...
                      {similar.average_rating > 0 && (
                        <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 rounded-full bg-white/90 backdrop-blur-sm">
                          ...
                          <span className="text-xs font-bold">{similar.average_rating.toFixed(1)}</span>
                        </div>
                      )}
                    </div>
                    <div className="p-4">
                      ...
                      <p className="text-xl font-black text-gray-900">GYD {similar.price_per_month?.toLocaleString()}<span className="text-xs text-gray-500 font-normal">/mo</span></p>
                    </div>
                  </Link>
                )
              })}
