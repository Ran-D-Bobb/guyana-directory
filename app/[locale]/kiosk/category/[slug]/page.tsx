...
export default async function Page({ params }: PageProps) {
  ...
  const transformedExperiences = experiences.map((exp) => {
    ...
    return {
      ...
      video_url: (exp as Record<string, unknown>).video_url as string | null ?? null,
      video_thumbnail_url: (exp as Record<string, unknown>).video_thumbnail_url as string | null ?? null,
    }
  })

  // Transform for new component interface
  const kioskExperiences = transformedExperiences.map(exp => ({
    ...exp,
