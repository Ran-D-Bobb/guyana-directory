...
export async function generateMetadata({
  ...
  if (!category) {
    return {
      title: `${t('notFound')} | Waypoint`,
    }
  }

  return {
    title: t('rentalsInGuyana', { name: category.name }),
    description: category.description || t('businessesDesc', { name: category.name }),
  }
}
...
export default async function RentalCategoryPage({
  ...
  if (filters.q) {
    const searchTerm = filters.q.replace(/[^a-zA-Z0-9\s\-']/g, ' ').trim()
    if (searchTerm) {
          ...
          <div className="bg-white border-b">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-bold text-gray-900">{getLocalizedName(category, locale)}</h1>
              </div>
              {getLocalizedDescription(category, locale) && (
                <p className="text-gray-600 mb-4">{getLocalizedDescription(category, locale)}</p>
              )}
