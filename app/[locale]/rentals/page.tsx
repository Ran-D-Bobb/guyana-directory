...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t('pageTitle'),
    description: 'Find the best accommodation in Guyana. Browse verified apartments, guesthouses, vacation homes, and lodges across Georgetown and beyond.',
  }
}
...
export default async function RentalsPage({
  ...
  const effectiveRegion = params.region || 'all'
  ...
  // Apply search filter
  const safeSearchTerm = params.q ? params.q.replace(/[^a-zA-Z0-9\s\-']/g, ' ').trim() : ''
  if (safeSearchTerm) {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-900">
                    {currentCategory?.name || t('allStays')}
                  </h2>
                  <p className="text-sm text-gray-600">
                    {t('listingCount', { count: totalCount || 0 })}
                  </p>
                </div>
              </div>
            </div>
            ...
            {hasFilters && (
              <div className="mb-8 animate-fade-up">
                <h1 className="font-display text-3xl lg:text-4xl text-gray-900 mb-2">
                  {currentCategory?.name || t('searchResultsHeading')}
                </h1>
                <p className="text-gray-600 text-lg">
                  {params.q ? t('resultsFor', { query: params.q }) : t('staysMatchingSearch')}
                </p>
              </div>
            )}
              ...
              <div className="mt-12 animate-fade-up" style={{ animationDelay: '1000ms' }}>
                ...
                <p className="text-center text-sm text-gray-500 mt-4">
                  {t('pagination', {
                    ...
                    to: Math.min(currentPage * ITEMS_PER_PAGE, totalRentals),
                    total: totalRentals,
                  })}
                </p>
              </div>
