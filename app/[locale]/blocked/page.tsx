...
function BlockedContent() {
  ...
  const t = useTranslations('metadata.blocked')
  ...
  return (
    <div className="text-center">
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-2">
        {status === 'banned' ? t('banned') : t('suspended')}
      </h2>

      <div className="text-gray-600 mb-6 space-y-2">
        {status === 'banned' ? (
          <p>{t('permanentlyBanned')}</p>
        ) : (
          <>
            {expiresAt ? (
              isExpired ? (
                <p>{t('suspensionExpired')}</p>
              ) : (
                <p>{t('suspendedUntil', { date: formatExpiryDate(expiresAt) })}</p>
              )
            ) : (
              <p>{t('suspendedIndefinitely')}</p>
            )}
          </>
        )}
