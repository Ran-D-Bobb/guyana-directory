...
// Revalidate every 10 minutes — directory data changes infrequently
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  // Richer meta description: business description + category + region (target 120-160 chars)
  const description = business.description
    ? `${business.description.slice(0, 120)}${categoryName ? ` | ${categoryName}` : ''} in ${regionName || 'Guyana'}.`
    : `${business.name} is a ${categoryName || 'business'} in ${regionName || 'Guyana'}. Find contact details, reviews, hours, and photos on Waypoint.`
  ...
  return {
    ...
    openGraph: {
      ...
      description: description.slice(0, 160),
      url: `https://waypointgy.com/businesses/${slug}`,
      ...(photo ? { images: [{ url: photo, width: 1200, height: 630, alt: `${business.name} - ${categoryName} in ${regionName}` }] } : {}),
    },
  }
}
...
function formatPhoneE164(phone: string): string {
  ...
  if (digits.startsWith('592')) return `+${digits}`
  if (digits.length <= 7) return `+592-${digits}`
  return `+592-${digits}`
}

function buildBreadcrumbJsonLd(business: Record<string, unknown>) {
  const category = business.categories as { name?: string; slug?: string } | null
  const items = [
    { position: 1, name: 'Home', item: 'https://waypointgy.com' },
    { position: 2, name: 'Businesses', item: 'https://waypointgy.com/businesses' },
    ...(category?.slug ? [{
      position: 3,
      name: category.name || 'Category',
      item: `https://waypointgy.com/businesses/category/${category.slug}`,
    }] : []),
    { position: category?.slug ? 4 : 3, name: business.name as string },
  ]
...
function buildJsonLd(business: Record<string, unknown>, heroImage: string, businessHours: BusinessHours | null) {
  return {
    '@context': 'https://schema.org',
    '@type': 'LocalBusiness',
    '@id': `https://waypointgy.com/businesses/${business.slug}#business`,
    name: business.name,
    description: business.description || undefined,
    url: `https://waypointgy.com/businesses/${business.slug}`,
    image: heroImage,
    telephone: business.phone ? formatPhoneE164(business.phone as string) : undefined,
...
export default async function BusinessPage({ params }: BusinessPageProps) {
  ...
  // Stage 1: Fetch user auth (cached, shared with Header) and business data in parallel
  const [user, businessResult] = await Promise.all([
    getUser(),
    supabase
      .from('businesses')
      .select(`
  ...
  // Always derive the displayed review count from the actual reviews table, not the
  // (potentially stale, Google-scraped) business.review_count field.
  // The RPC queries the reviews table directly; fall back to the fetched page length.
  const displayTotalReviews = dbRatingCounts
    ? ((dbRatingCounts[0]?.count_5 ?? 0) + (dbRatingCounts[0]?.count_4 ?? 0) + (dbRatingCounts[0]?.count_3 ?? 0) + (dbRatingCounts[0]?.count_2 ?? 0) + (dbRatingCounts[0]?.count_1 ?? 0))
    : reviews.length
  ...
  const breadcrumbJsonLd = buildBreadcrumbJsonLd(business)
      ...
      <div className="min-h-screen bg-[hsl(var(--jungle-50))]">
        ...
        <div className="fixed top-[60px] md:top-[82px] left-3 md:left-4 z-40 animate-fade-up">
          ...
          >
            ...
            <span className="text-xs md:text-sm font-medium">{t('back')}</span>
          </Link>
        </div>
              ...
              <div className="flex flex-wrap items-center gap-3 md:gap-4 text-sm md:text-base text-white/90 text-hero-body animate-fade-up delay-300">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">
                    {(business.rating ?? 0) > 0 ? (business.rating ?? 0).toFixed(1) : t('newRating')}
                  </span>
                  {displayTotalReviews > 0 && (
                    <span className="text-white/60">
                      ({t('reviewsCount', { count: displayTotalReviews })})
                    </span>
                  )}
                </div>
              ...
              <div className="lg:col-span-2 space-y-5">
                  ...
                  <div className="flex items-center justify-center gap-5">
                    {business.phone && (
                      <a href={`tel:${business.phone}`} className="flex flex-col items-center gap-1.5 min-w-[52px] group">
                        ...
                        <span className="text-[11px] font-medium text-[hsl(var(--jungle-700))]">{t('call')}</span>
                      </a>
                    )}
                    {business.email && (
                      <a href={`mailto:${business.email}`} className="flex flex-col items-center gap-1.5 min-w-[52px] group">
                        ...
                        <span className="text-[11px] font-medium text-[hsl(var(--jungle-700))]">{t('email')}</span>
                      </a>
                    )}
                    {business.website && (
                      <a href={business.website} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1.5 min-w-[52px] group">
                        ...
                        <span className="text-[11px] font-medium text-[hsl(var(--jungle-700))]">{t('web')}</span>
                      </a>
                    )}
                    <div className="flex flex-col items-center gap-1.5 min-w-[52px]">
                      ...
                      <span className="text-[11px] font-medium text-[hsl(var(--jungle-700))]">{t('save')}</span>
                    </div>
                    <ShareButton
                      title={business.name}
                  ...
                  {business.address && (
                    <div className="mt-6 pt-6 border-t border-[hsl(var(--border))]/60">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-medium text-[hsl(var(--jungle-800))] text-sm">{t('address')}</p>
                ...
                {businessEvents && businessEvents.length > 0 && (
                  <article className="bg-white rounded-2xl shadow-sm shadow-black/[0.03] border border-[hsl(var(--border))]/60 p-6 lg:p-8 animate-fade-up delay-300">
                    ...
                    >
                      {t('viewAllEvents', { businessName: business.name })}
                    </Link>
                  </article>
                )}
                    ...
                    <>
                      ...
                      {displayTotalReviews > reviews.length && (
                        <LoadMoreReviews
                          ...
                          totalCount={displayTotalReviews}
                        />
                      )}
                    </>
                  ) : (
                    ...
                    <div className="space-y-1">
                      {business.phone && (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-[10px] uppercase tracking-wider text-[hsl(var(--muted-foreground))] font-medium">{t('call')}</p>
                      ...
                      {business.email && (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-[10px] uppercase tracking-wider text-[hsl(var(--muted-foreground))] font-medium">{t('email')}</p>
                      ...
                      {business.website && (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] uppercase tracking-wider text-[hsl(var(--muted-foreground))] font-medium">{t('web')}</p>
