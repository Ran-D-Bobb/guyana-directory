...
export default async function EventPage({ params }: EventPageProps) {
  ...
  // Parallelize auth (cached, shared with Header) + event fetch
  const [user, eventResult] = await Promise.all([
    getUser(),
    supabase
      .from('events')
      .select(`
        id, title, slug, description, image_url, location, event_type,
        start_date, end_date,
        is_featured, user_id,
        view_count, interest_count, whatsapp_clicks,
  ...
  // Parallelize creator profile, interest check, follow check, and related events (all independent)
  const [creatorProfileResult, interestResult, followResult, relatedEventsResult] = await Promise.all([
    event.user_id
    ...
    user
      ? (event.businesses?.id
          ? supabase.from('followed_organisers').select('id').eq('user_id', user.id).eq('business_id', event.businesses.id).single()
          : event.user_id
            ? supabase.from('followed_organisers').select('id').eq('user_id', user.id).eq('organiser_user_id', event.user_id).single()
            : Promise.resolve({ data: null }))
      : Promise.resolve({ data: null }),
  ...
  const isFollowingOrganiser = !!followResult.data
            ...
            <div className="lg:col-span-2 space-y-6 animate-slide-in">
                ...
                <div className="flex flex-wrap gap-4 pt-6 border-t-2 border-gray-100">
                  <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
                    ...
                    <span className="text-sm font-semibold text-gray-900">
                      {t('views', { count: event.view_count ?? 0 })}
                    </span>
                  </div>
                  {(event.interest_count ?? 0) > 0 && (
                    <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
                      ...
                      <span className="text-sm font-semibold text-gray-900">
                        {t('interested', { count: event.interest_count ?? 0 })}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-md p-8">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                      <span className="w-1 h-8 bg-gradient-to-b from-emerald-600 to-amber-500 rounded-full" />
                      {t('moreEvents', { categoryName: event.event_categories?.name || '' })}
                    </h2>
                    {event.event_categories && (
                      <Link
              ...
              {isPast && (
                <div className="bg-white rounded-2xl shadow-md p-8 text-center">
                  <p className="text-gray-600 mb-4">{t('pastEventBody')}</p>
                  <Link
                    href="/events"
                  ...
                  {event.businesses ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <FollowOrganiserButton
                        ...
                        organiserName={event.businesses.name}
                        initialIsFollowing={isFollowingOrganiser}
                        userId={user?.id || null}
                        variant="icon-label"
                      ...
                      <div className="flex items-start gap-4 p-4 rounded-xl bg-gradient-to-br from-emerald-50 to-amber-50 border border-emerald-100">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-emerald-600 font-medium">
                            {t('communityOrganizer')}
                          </p>
                        </div>
                      </div>

                      <FollowOrganiserButton
                        organiserUserId={event.user_id}
                        organiserName={creatorProfile.name || 'Organizer'}
                        initialIsFollowing={isFollowingOrganiser}
                        userId={user?.id || null}
                        variant="icon-label"
                        className="w-full justify-center"
                    ...
                    <p className="text-sm text-gray-500 italic p-3 bg-gray-50 rounded-lg text-center">{t('organizerUnavailable')}</p>
                  )}
                </div>
              </div>
            </div>
