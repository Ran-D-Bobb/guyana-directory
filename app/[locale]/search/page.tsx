...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t('pageTitle'),
    description: 'Search for businesses, experiences, events, and stays across Guyana.',
...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const searchTerm = q.trim().replace(/[^a-zA-Z0-9\s\-']/g, ' ').trim()
      ...
      <header className="border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-6 py-6">
          ...
          {searchTerm ? (
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{t('inputLabel')}</h1>
          )}
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        {!searchTerm ? (
          ...
          <div className="text-center py-16 animate-fade-up">
            ...
            <p className="text-gray-900 text-lg font-medium mb-2">{t('noResultsHeading')}</p>
